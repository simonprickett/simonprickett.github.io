...
The other fun thing here, I am SSH'ed into `bussign.local` on my network because there's a Pi in there - I'm SSH'ed into the bus sign and I can run stuff on it.  Let's run the front end of the plane tracking project ([full article on this project](/plane-spotting-with-redis-nodejs-micropython/))...
